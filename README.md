...
- **离线可用。** 所有前端依赖都已本地化，没有远程字体，克隆下来用任何静态服务器都能跑。
...
| 位置 | 功能 |
| --- | --- |
| 顶栏 | 中英 / 中文切换，阅读设置（主题、字号），全书搜索 |
| 左栏 | 章节列表 |
| 右栏 | 本章目录，随滚动高亮当前小节 |
| 章首 | 本章图解，可展开收起，支持放大、适应窗口、下载 SVG |
| 正文 | 小节图解内嵌显示，脚注和译者注点击弹出 |
| 章末 | 上一章 / 下一章，讨论区按需加载 |
...
assets/reader-manifest.json   章节与图解清单
assets/reader-content/    每章的脚注、译者注和展示层修订
...
改动图解或译者注后，用 `uv run python build_reader_manifest.py` 重新生成并校验阅读器清单。
...
书籍内容采用 [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/) 授权，代码采用 [BSD 3-Clause](https://opensource.org/licenses/BSD-3-Clause) 许可。阅读器使用的 [Docsify](https://docsify.js.org/) 与 [Lucide](https://lucide.dev/) 的许可见 [`assets/vendor/`](assets/vendor/)。
