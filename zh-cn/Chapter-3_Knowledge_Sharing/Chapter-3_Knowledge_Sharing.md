...
那么，假如真有一个神奇的世界，所有知识都能立即、完整地记录下来，我们就不必再向人请教了，对吗？并非如此。书面知识有利于扩大知识传播的规模，有针对性的人工帮助也一样。专家能够融会贯通自己掌握的广泛知识，判断哪些信息适合提问者的具体场景、文档是否仍然适用，以及到哪里去找。即使不知道答案在哪里，他们也可能知道谁能找到答案。[^t1]
...
[^1]: 换句话说，我们得到的不是一个全局最大值，而是一组局部最大值。

[^2]: David Lorge Parnas，《软件工程：由多人开发多版本程序》（海德堡：Springer-Verlag Berlin，2011）。

[^3]: 冒名顶替综合征在成就出众的人群中并不少见，谷歌员工也不例外。事实上，本书的大多数作者都有这种感受。我们理解，对这些人来说，害怕失败可能很难克服，也可能使他们更不愿意涉足新的领域。

[^4]: 见“如何提出好问题”。

[^5]: https://talksat.withgoogle.com 和 [https://www.youtube.com/GoogleTechTalks](https://www.youtube.com/GoogleTechTalks)，仅举几例。

[^6]: g2g 计划详见 Laszlo Bock 的《工作规则！：来自谷歌内部、将改变你生活与领导方式的洞见》（纽约：Twelve Books，2015）。书中介绍了该计划的各个方面、如何评估其影响，以及建立类似计划时应重点关注什么。

[^7]: 见“童子军规则”，以及 Kevlin Henney 的《每个程序员都应该知道的97件事》（波士顿：O'Reilly，2010）。

[^8]: g3doc 是“google3 documentation”的缩写。google3 是谷歌当前这套单体源代码仓库的名称。

[^9]: Laszlo Bock，《工作规则！：来自谷歌内部、将改变你生活与领导方式的洞见》（纽约：Twelve Books，2015）。

[^10]: 同上。

[^11]: 同事奖金包括现金奖励和证书，还会永久保存在内部工具 gThanks 的员工获奖记录中。
...
[^15]: 涵盖多种编程语言，其中 C++ 的内容已公开，可在[https://abseil.io/tips](https://abseil.io/tips)查阅。

[^16]: go/ 链接与 Go 语言无关。

[^17]: 对外开放的 codelab 可在 `https://codelabs.developers.google.com` 获取。

[^18]: 变更列表是版本控制系统中构成一次变更的文件清单，与变更集同义。

[^19]: 截至2019年，仅谷歌 C++ 风格指南就有40页。构成完整最佳实践资料集的配套材料，篇幅还要长数倍。

[^20]: 关于谷歌采用单体代码仓库的原因，请参阅 https://cacm.acm.org/magazines/2016/7/204032-why-google-stores-billions-of-lines-of-code-in-a-single-repository/fulltext. 另请注意，谷歌并非所有代码都在这个仓库中；这里介绍的 Readability 只适用于该单体代码仓库，因为它关注的是仓库内部的一致性。

[^21]: 因此，明确只会短期使用的代码可以豁免 Readability 要求。例如 experimental/ 目录中的代码（明确用于实验，不能发布到生产环境），以及 Area 120 项目中的代码；后者是谷歌实验性产品的孵化项目。

[^22]: 研究控制了多种因素，包括在谷歌的任职时间，以及尚未取得 Readability 资格的作者所提交的 CL，通常比已有资格的作者需要更多轮审查这一事实。

[^t1]: **补充释义：口头知识** tribal knowledge：口头知识；是指一种仅存在于某个部落中的信息或知识， 这些知识不为外界所知，没有正式记录， 只能口口相传。
