...
class MarkdownTests(unittest.TestCase):
    ...
    def test_term_lines_without_break_are_reported(self):
        self.assertEqual(audit.term_lines_without_break("- *Term*\n    Description.\n- **Other**  \n    Fine.\n"), [1])
        self.assertEqual(audit.term_lines_without_break("*Fast*\n    Text.\n*Correct*\n    Text.\n"), [1, 3])
    ...
    def test_book_terms_break_before_their_description(self):
        for path in audit.book_files(audit.ROOT):
            ...
            self.assertEqual(audit.term_lines_without_break(path.read_text()), self.EPIGRAPH_LINES.get(file, []), file)
    ...
    def test_book_chinese_emphasis_markers_all_pair(self):
        for path in audit.book_files(audit.ROOT):
            ...
            chinese = [n for n in audit.literal_emphasis_markers(path.read_text()) if audit.HAN.search(lines[n - 1])]
            self.assertEqual(chinese, self.LITERAL_ASTERISKS.get(file, []), file)
